...
void sync(std::vector<VCDataBase>& vcdb, const std::string &filename)
{
    ...
    const char *sqlCreateTable = "CREATE TABLE IF NOT EXISTS VCDB ("
                                 ...
                                 "City TEXT NOT NULL);";
    retval = sqlite3_exec(db, sqlCreateTable, nullptr, nullptr, nullptr);
    if(retval)
    {
        sqlite3_close(db);
        return;
    ...
    sqlite3_exec(db, "SELECT DISTINCT Name, Telephone, Address, City FROM VCDB;", nullptr, nullptr, nullptr);
